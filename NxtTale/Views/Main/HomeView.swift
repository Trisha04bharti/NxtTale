...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    VStack(alignment: .leading, spacing: 28) {
                        ...
                        HStack(alignment: .top) {
                            VStack(alignment: .leading, spacing: 2) {
                                Text("NxtTale").font(.largeTitle.bold())
                                Text("Every Story Begins Somewhere")
                                    .font(.subheadline)
                                    .foregroundColor(.secondary)
                            }
                            Spacer()
                            Button {
                                withAnimation(.spring()) { showProfilePopup = true }
                            } label: {
                                VStack(spacing: 4) {
                                    ZStack {
                        ...
                        if !vm.recommendations.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                HStack {
                                    ...
                                    Text("Picked For You").font(.title3.bold())
                                    Spacer()
                                }
                                .padding(.horizontal)
                        ...
                        if !vm.feedBooks.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                Text("All Books").font(.title3.bold()).padding(.horizontal)
                                LazyVStack(spacing: 0) {
                                    ForEach(vm.feedBooks) { book in
                                        BookRowView(book: book).padding(.horizontal)
            ...
            // ── Reload every time home appears (back nav too) ──
            .onAppear {
                Task {
                    await vm.loadAll(token: authVM.token)
                }
            ...
            // ── Reload instantly when any book is opened ──
            .onReceive(
                NotificationCenter.default.publisher(for: .activityTracked)
            ) { _ in
                Task {
                    await vm.loadAll(token: authVM.token)
                }
