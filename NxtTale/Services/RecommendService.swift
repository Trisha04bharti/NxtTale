...
class RecommendService {
    ...
    // ── Fetch recommendations (cache busted) ──
    func getRecommendations(token: String) async throws -> RecommendResponse {
        let url = URL(string: "\(base)/recommend?t=\(Int(Date().timeIntervalSince1970))")!
        var req = URLRequest(url: url)
        req.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
    ...
    // ── Track activity then notify home to reload ──
    func trackActivity(
        token:        String,
        bookId:       String,
        googleBookId: String?,
...
extension Notification.Name {
    static let activityTracked = Notification.Name("activityTracked")
}
